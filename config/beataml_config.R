...
# Purpose:      Configurations for the Beat AML dataset
# Author:       Sophia Li
...
config <- list(
  ...
  general = list(
    dataset_name             = "beataml",         # Dataset name for directories
    n_cores                  = 40,                # Parallel computation cores
    genome_build             = "hg19",            # Genome build of data
    seed                     = 123,               # Seed for random generation
    slurm_scratch_path       = "/scratch/..."     # SLURM cluster path to SCRATCH
  ),
  
  # --- Cohort definitions ---
  cohorts = list(
    reference                = "PMLRARA",               # Reference cohort
    ...
    # A) Group 1
    group_1 = list(
      name                   = "NPM1",                     # Name of cohort 1
      column                 = "specificDxAtAcquisition",  # Clinical data column
      pattern                = "NPM1"                      # Pattern name
    ),
    
    # B) Group 2
    group_2 = list(
      name                   = "PMLRARA",                  # Name of cohort 2
      column                 = "consensusAMLFusions",      # Clinical data column
      pattern                = "PML-RARA"                  # Pattern name
    )
  ),
  ...
  methylation = list(
    ...
    preproc = list(
      ...
      normalize_beta        = FALSE,             # Normalize beta values
